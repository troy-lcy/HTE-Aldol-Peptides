...
def oversample(X_train, y_train, X, label_df, high_scale = 0, medium_scale = 0, low_scale = 0):
    ...
    if medium_scale == 0:
        X_high = X.iloc[high_X].copy()
        y_high = label_df.iloc[high_X].copy()
        X_high = pd.concat([X_high]*high_scale)  # change the number to adjust oversampling scale
        y_high = pd.concat([y_high]*high_scale)

        X_train = pd.concat([X_train,X_high], axis=0)
        y_train = pd.concat([y_train,y_high], axis=0).astype(int)
    else:
        X_high = X.iloc[high_X].copy()
        y_high = label_df.iloc[high_X].copy()
        X_high = pd.concat([X_high]*high_scale)  # change the number to adjust oversampling scale
        ...
        X_medium = X.iloc[medium_X].copy()
        y_medium = label_df.iloc[medium_X].copy()
        X_medium = pd.concat([X_medium]*medium_scale)
        y_medium = pd.concat([y_medium]*medium_scale)

        X_train = pd.concat([X_train,X_high,X_medium], axis=0)
        y_train = pd.concat([y_train,y_high,y_medium], axis=0).astype(int)
    
    if low_scale != 0:
        X_low = X.iloc[low_X].copy()
        y_low= label_df.iloc[low_X].copy()
        X_low = pd.concat([X_low]*high_scale)  
        y_low = pd.concat([y_low]*high_scale)

        X_train = pd.concat([X_train,X_low], axis=0)
        y_train = pd.concat([y_train,y_low], axis=0).astype(int)
